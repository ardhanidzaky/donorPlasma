...
def get_prov_by_name(prov):
    if not prov:
        return None
    prov = Provinsi.objects.get(nama=prov)
    return prov

def get_kota_by_name(kota):
    if not kota:
        return None
    kota = Kota.objects.get(nama=kota)
    return kota

# def get_prov_by_name(request):
#     prov = request.GET.get('name', None)
#     if not prov:
...
# def get_kota_by_name(request):
#     kota = request.GET.get('name', None)
#     if not kota:
#         return HttpResponse("None", content_type="application/json")
#     kota = Kota.objects.get(nama=kota)
